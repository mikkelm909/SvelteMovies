...
<script context="module">
    import { loadPopular } from "$lib/header/Stores/Stores";   
    export async function load({fetch}){
        //Viser kun at komponenten bliver hentet efter load er færdig
        //await new Promise(resolve => setTimeout(resolve, 2000))
        const res = await fetch ('http://api.themoviedb.org/3/discover/movie?sort_by=popularity.desc&api_key=81c50c197b83129dd4fc387ca6c8c323')
        ...
        return {
            ////Commenteret brug den til at sende fejl til fejsiden / ukommenteret kode redirects koden til home-page
            // status: res.status,
  ...
  /*        // Bruger store med loadPopular()
    export async function load(){
        return {
            props: {pop: loadPopular()}
        }
    } 
    */
</script>
