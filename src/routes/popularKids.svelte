<script context="module">
    export async function load(){
        const result =  await fetch('http://api.themoviedb.org/3/discover/movie?certification_country=US&certification.lte=G&sort_by=popularity.desc&api_key=81c50c197b83129dd4fc387ca6c8c323');
        const data = await result.json()
        return {
            props: {popularKids: data.results}
        }
    }
</script>

<script>
    import { loadPopularKidsMovies } from "$lib/header/Stores/Stores";
    import { fade } from "svelte/transition";
    export let popularKids
    
    const movies = loadPopularKidsMovies()
</script>
